...
mixin EventRepository {
  ...
  static final eventsCol = _firestore.collection('events');
  static DocumentReference eventDoc(String eventID) => eventsCol.doc(eventID);
  static final reportedEventsCol = _firestore.collection('reported_events');
  static DocumentReference reportedDoc(String id) => reportedEventsCol.doc(id);
  //SECTION ------------------------------events
  static DocumentSnapshot lastEventDoc;
  static Future<List<Post>> fetchEvents(int page) async {
    var col = eventsCol.limit(20).orderBy('createdAt', descending: true);
    if (page != 0 && lastEventDoc != null) {
      col = col.startAfterDocument(lastEventDoc);
    }
    final docs = (await col.get()).docs;
    if (docs.isNotEmpty) lastEventDoc = docs.last;
    return docs.map((s) => Post.fromMap(s.data())).toList();
  }

  static Stream<Post> singleEventStream(String eventID) {
    return eventDoc(eventID).snapshots().map(
...
//SECTION ------------------------------Post Report
  ...
  static Stream<List<Post>> reportedPostsStream() =>
      reportedEventsCol.orderBy('id', descending: true).snapshots().map(
