...
class PostAdsEditorPage extends StatefulWidget {
  final PostAds toEditPostAds;
  const PostAdsEditorPage({
    Key key,
    this.toEditPostAds,
  }) : super(key: key);
...
class _PostEditorPageState extends State<PostAdsEditorPage> {
  final textController = TextEditingController();

  PostAds get toEdit => widget.toEditPostAds;
  ...
  Future<void> onShareTap() async {
    ...
    if (toEdit != null) {
      ...
      await PostAdsRepository.updatePost(toEdit.copyWith(content: content));
      BotToast.showText(text: t.SuccessPostEdited);
    } else {
      var post = PostAds.create(
        content: content,
      );
        ...
        uploader.picked.when(
          image: (img) {
            ...
            uploader.upload(
              ...
              onSuccess: (v) async {
                await PostAdsRepository.addPost(post.copyWith(
      ...
      await PostAdsRepository.addPost(post);
    }
    Get.find<PagingController>().refresh();
    Navigator.pop(context);
  }
  ...
  Future<void> deletePost() async {
    try {
      await PostAdsRepository.removePost(toEdit.id, toEdit.commentsIDs);
      Get.find<PagingController>().refresh();
    } catch (e) {
      logError(e);
    }
